...
class ProgressExtension implements Extension, Observable {
    ...
    public function setProgressBarSize($charWidth) {
    ...
    public function setProgressBarIncrementChar($char) {
    ...
    public function setProgressBarEmptyIncrementChar($char) {
        if (is_string($char) && strlen($char) === 1) {
    ...
    public function setProgressBarLeadingChar($char) {
        if (is_string($char) && strlen($char) === 1) {
    ...
    public function extend(Observable $client) {
        $this->unextend();
        $this->observation = $client->addObservation([
            Event::REQUEST => function($dataArr) { $this->onRequest($dataArr); },
            Event::SOCKET => function($dataArr) { $this->onSocket($dataArr); },
            Event::SOCK_DATA_IN => function($dataArr) { $this->onData($dataArr); },
            Event::HEADERS => function($dataArr) { $this->onHeaders($dataArr); },
            Event::REDIRECT => function($dataArr) { $this->onRedirect($dataArr); },
            Event::RESPONSE => function($dataArr) { $this->onResponse($dataArr); },
            Event::CANCEL => function($dataArr) { $this->clear($dataArr); },
            Event::ERROR => function($dataArr) { $this->onError($dataArr); }
        ]);
